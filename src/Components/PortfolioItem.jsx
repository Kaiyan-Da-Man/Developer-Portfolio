        ...
        <a href={link} target='_blank'>
           <img 
            ...
            <div className='w-full p-4'>
                <h3 className='text-lg md:text-xl mb-2 md:mb-3 font-semibold dark:text-lightYellow'>{title}</h3>
                <p className='flex flex-wrap gap-2 flex-row items-center justify-start text-xs md:text-sm dark:text-springGreen-50'>
                    ...
                    {stack.map(item => (
                        <span className='inline-block px-2 py-1 font-semibold border-2 border-myTeal-dark dark:border-lightYellow rounded-md'>
                            {item}
                        </span>
                    ))}
                </p>
